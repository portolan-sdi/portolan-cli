...
**owns** the bucket contents, so a partial or wrong push corrupts the
...
- Push uploads the full catalog: every `catalog.json`, `collection.json`, root
  ...
  MUST but is NOT generated by the CLI yet, do not assume push
...
`versions.json` stores **complete snapshots**, so naively uploading
...
- Within a collection, upload `versions.json` **last** (manifest-last
  atomicity), so a reader never sees a manifest referencing
