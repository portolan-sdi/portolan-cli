...
during ArcGIS layer extraction. The retry strategy follows the design:
...
class RetryConfig:
    ...
    def get_delay(self, attempt: int) -> float:
        ...
        Uses exponential backoff: delay = initial_delay * (backoff_factor ^ (attempt - 1))
...
class RetryResult(Generic[T]):
    ...
    def unwrap(self) -> T:
        ...
        Raises:
            RetryError: If the operation failed after all retries
        """
        if self.success:
...
def retry_with_backoff(
    ...
    **kwargs: Any,
    ...
    delay between attempts. On success, returns the result. On failure,
    returns a RetryResult with error information.
    ...
    for attempt in range(1, config.max_attempts + 1):
        try:
            ...
            # If we have more attempts, sleep and retry
            ...
            # Exception type not in retry_on - fail immediately
            return RetryResult(
                success=False,
                value=None,
                attempts=attempt,
